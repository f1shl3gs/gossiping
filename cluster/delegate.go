...
func newDelegate(l *zap.Logger, reg prometheus.Registerer, p *Peer, retransmit int) *delegate {
	bcast := &memberlist.TransmitLimitedQueue{
...
func (d *delegate) NotifyMsg(b []byte) {
	...
	if err := proto.Unmarshal(b, &p); err != nil {
		d.logger.Warn("decode broadcast failed",
			zap.Error(err))
		return
	}
...
func (d *delegate) LocalState(_ bool) []byte {
	...
	for key, s := range d.states {
		...
		if err != nil {
			d.logger.Warn("encode local state failed",
				zap.String("key", key),
				zap.Error(err))
			return nil
		}
...
func (d *delegate) MergeRemoteState(buf []byte, _ bool) {
	...
	if err := proto.Unmarshal(buf, &fs); err != nil {
		d.logger.Warn("merge remote state failed",
			zap.Error(err))
		return
	}
	d.mtx.RLock()
	...
	for _, p := range fs.Parts {
		s, ok := d.states[p.Key]
		if !ok {
			d.logger.Warn("unknown state key",
				zap.String("key", p.Key),
				zap.Int("len", len(buf)))
			continue
		}
		if err := s.Merge(p.Data); err != nil {
			d.logger.Warn("merge remote state",
				zap.String("key", p.Key),
				zap.Error(err))
			return
		}
...
func (d *delegate) handleQueueDepth() {
	for {
		...
		case <-time.After(15 * time.Minute):
			n := d.bcast.NumQueued()
			if n > maxQueueSize {
				d.logger.Warn("dropping messages because too many are queued",
					zap.Int("current", n),
					zap.Int("limit", maxQueueSize))
