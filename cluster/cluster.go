...
func Create(
	...
	l.Debug("resolved peers to following addresses",
		zap.Strings("peers", resolvedPeers))

	// Initial validation of user-specified advertise address.
	addr, err := calculateAdvertiseAddress(bindHost, advertiseHost)
	if err != nil {
		l.Warn("couldn't deduce an advertise address",
			zap.Error(err))
	} else if hasNonlocal(resolvedPeers) && isUnroutable(addr.String()) {
		l.Warn("this node advertises itself on an unroutable address",
		...
		l.Info("setting advertise address explicitly",
			zap.String("addr", addr.String()),
			zap.Int("port", bindPort))
...
func (p *Peer) Join(
	...
	if err != nil {
		p.logger.Warn("failed to join cluster",
			zap.Error(err))
		if reconnectInterval != 0 {
			p.logger.Info("will retry joining cluster repeatedly",
				zap.Duration("interval", reconnectInterval))
		}
	} else {
		p.logger.Debug("joined cluster",
			zap.Int("peers", n))
	}
...
func (p *Peer) removeFailedPeers(timeout time.Duration) {
	...
	for _, pr := range p.failedPeers {
		if pr.leaveTime.Add(timeout).After(now) {
			...
			p.logger.Debug("failed peer has timed out",
				zap.String("peer", pr.Name),
				zap.String("addr", pr.Address()))
...
func (p *Peer) reconnect() {
	...
	for _, pr := range failedPeers {
		...
		if _, err := p.mlist.Join([]string{pr.Address()}); err != nil {
			...
			p.logger.Debug("reconnect failure",
				zap.String("peer", pr.Node.Name),
				zap.String("addr", pr.Address()),
				zap.Error(err))
		} else {
			p.reconnectionsCounter.Inc()
			p.logger.Debug("reconnect success",
				zap.String("peer", pr.Name),
				zap.String("addr", pr.Address()))
		}
	}
...
func (p *Peer) refresh() {
	...
	if err != nil {
		p.logger.Debug("refresh",
			zap.Strings("peers", p.knownPeers),
			zap.Error(err))
		return
	}
	...
	for _, peer := range resolvedPeers {
		...
		if !isPeerFound {
			if _, err := p.mlist.Join([]string{peer}); err != nil {
				...
				p.logger.Warn("refresh failure",
					zap.String("addr", peer),
					zap.Error(err))
			} else {
				p.refreshCounter.Inc()
				p.logger.Debug("refresh success",
					zap.String("addr", peer))
			}
		}
	}
...
func (p *Peer) Settle(ctx context.Context, interval time.Duration) {
	...
	p.logger.Info("waiting for gossip to settle",
	...
	for {
		...
		case <-ctx.Done():
			...
			p.logger.Info("gossip not settled but continuing anyway",
				zap.Int("polls", totalPolls),
		...
		if nOkay >= NumOkayRequired {
			p.logger.Info("gossip settled and proceeding",
				zap.Duration("elapsed", elapsed))
			break
		}
		if n == nPeers {
			nOkay++
			p.logger.Debug("gossip looks settled",
				zap.Duration("elapsed", elapsed))
		} else {
			nOkay = 0
			p.logger.Info("gossip not settled",
				zap.Int("polls", totalPolls),
				zap.Int("before", nPeers),
				zap.Int("now", n),
				zap.Duration("elapsed", elapsed))
		}
		nPeers = n
